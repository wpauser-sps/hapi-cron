...
internals.trigger = (server, job) => {
    ...
    return async () => {
        ...
        if (job.shouldSkip()) {
            server.log([PluginPackage.name], `Skipping: ${job.name}`);
            return;
        }
...
const PluginRegistration = (server, options) => {
    ...
    if (!options.jobs || !options.jobs.length) {
        ...
        options.jobs.forEach((job) => {
            ...
            if (typeof job.shouldSkip === 'undefined') {
                job.shouldSkip = () => false;
            }
            if (typeof job.shouldSkip === 'boolean') {
                const shouldSkip = job.shouldSkip;
                job.shouldSkip = () => shouldSkip;
            }
            Hoek.assert(typeof job.shouldSkip === 'function' || typeof job.onComplete === 'undefined', 'shouldSkip value must be a function or boolean');
